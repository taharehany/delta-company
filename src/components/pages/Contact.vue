<template lang="pug">
//if mesage sent
Toast(:showIfValue="sent" :message="message_sent_successfully" :success="true")

//if mesage not sent
Toast(:showIfValue="recapchaError" :message="recapcha_error" :success="false")
                        ...
                        div(class="row")
                            div(class="col-lg-6")
                                div(class="form-group input-group")
                                    input(type="text", class="form-control", placeholder="name" name="name" v-model="contactFormData.name" )
                                    div(class="invalid-feedback" v-if="contactErrors.name && !sent" v-for="error in contactErrors.name") {{ error }}
                            div(class="col-lg-6")
                                div(class="form-group input-group")
                                    input(type="text", class="form-control", placeholder="phone" name="phone" v-model="contactFormData.phone")
                                    div(class="invalid-feedback" v-if="contactErrors.phone && !sent" v-for="error in contactErrors.phone") {{ error }}
                            div(class="col-lg-12")
                                div(class="form-group input-group")
                                    input(type="text", class="form-control", placeholder="email" name="email" v-model="contactFormData.email")
                                    div(class="invalid-feedback" v-if="contactErrors.email && !sent" v-for="error in contactErrors.email") {{ error }}
                            div(class="col-lg-12")
                                div(class="form-group input-group")
                                    textarea(rows="6" class="form-control" placeholder="message" name="message" v-model="contactFormData.message")
